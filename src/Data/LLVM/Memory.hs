...
{-
 -- | Returns number of bytes for an L.Type
llvmByteSizeOf :: LLVMContext -> L.Type -> Integer
llvmByteSizeOf lc tp =
  case tp of
    L.PrimType pt            -> llvmPrimSizeOf pt
    L.Alias a                -> llvmByteSizeOf lc (llvmLookupAlias lc a)
    L.Array l eTp            -> toInteger l * llvmByteSizeOf lc eTp
    L.FunTy _retTp _argTpl _ -> error "internal: Cannot get size of function type."
    L.PtrTo _tp              -> toInteger (llvmAddrWidthBits lc `shiftR` 3)
    L.Vector l pt            -> toInteger l * llvmPrimSizeOf pt
    L.Opaque                 -> error "internal: Cannot get size of opaque type."
    --TODO: support alignment based on targetdata string in module.
    L.Struct argTypes        ->
      sum [ llvmByteSizeOf lc atp | atp <- argTypes ]
    L.PackedStruct argTypes  ->
      sum [ llvmByteSizeOf lc atp | atp <- argTypes ]
      -- NB: Alignment is always one byte with no padding between elements.
-}
...
-- TODO: Move to Data.LLVM.TargetData
